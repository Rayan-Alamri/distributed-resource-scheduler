...
7 & \texttt{ffmpeg\_time\_range} & Parallel FFmpeg processing of time ranges from one input video. \\
...
For video workloads, the dashboard supports two modes. In segment mode, the master or helper scripts split an input video into files such as \texttt{part\_000.mp4}, \texttt{part\_001.mp4}, and so on; each segment becomes a separate task. In time-range mode, the master queues tasks with a start second and duration, and workers read the same original input file while producing independent processed chunks. Workers write chunks into \texttt{videos/processed/}, and the master merges processed chunks into \texttt{videos/final/}. After a segment-mode task is successfully processed, its original input segment is deleted because it is no longer needed.
...
The video processing dialog is specialized for FFmpeg workflows. It lets the user choose an input video, select segment mode or time-range mode, set the chunk duration, and select the starting task ID. The dashboard then queues the appropriate FFmpeg tasks, tracks progress through the job panel, and merges the processed chunks after the job completes.
